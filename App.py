...
# -----------------------------------------------------------------------------
# 1. PAGE CONFIGURATION (Only called once!)
# -----------------------------------------------------------------------------
try:
...
try:
    ...
    pass # Will handle inside the specific pages if missing
...
<style>
    ...
    /* --- THEME CARD FIX FOR TESTIMONIALS & STATS --- */
    .white-card-fix {
        background-color: var(--background-color) !important;
...
""", unsafe_allow_html=True)
    ...
    def get_questions_json(board, cls, sub, chap, num, diff, q_type):
        ...
        try:
            ...
            clean_json = clean_json_response(raw_content) # Using the new regex helper
                ...
                if not all_answered and st.session_state.mt_q_type == "MCQ":
                    ...
                    with st.spinner("Grading..."):
                        try:
                            ...
                            st.error("Failed to grade. Please try again.")
